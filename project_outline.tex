...
In this report we investigate the middle ground between the two approaches for the domain of 9x9 Sudokus. Namely, we do not use a T-solver to d to thade T-box but check the clauses learnt by the SAT-solver on specific Sudokus for global validity on Sudokus. If a valid clause is found, it is added to the axioms.
...
A large part of the literature on Sudoku as a SAT-problem is devoted to optimize CNF-encodings for Sudoku with respect to SAT-solver performance. \citeA{sudokusat} suggest and test two encodings: the minimal and the extended encoding
 with respect SAT-solvers' ability to solve Sudokus wihtout search 
...
\subsection{Sudoku Encodings and DIMACS} \label{encodings}

\subsection{The Minisat-Solver} \label{minisat}

\subsection{Measures of Performance} \label{measures}
